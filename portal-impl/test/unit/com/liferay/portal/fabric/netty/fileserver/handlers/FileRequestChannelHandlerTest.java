...
public class FileRequestChannelHandlerTest {
	...
	public void testFile() throws IOException {
		...
		_embeddedChannel.writeInbound(new FileRequest(path, 0));
		...
		Assert.assertEquals(
			new FileResponse(
				path, Files.size(path), fileTime.toMillis(), false),
			queue.poll());
	...
	public void testFileNotFound() throws IOException {
		Path path = FileServerTestUtil.createMissingFile(
	...
	public void testFileNotModified() throws IOException {
		...
		_embeddedChannel.writeInbound(
			new FileRequest(path, fileTime.toMillis()));
	...
	public void testFolder() throws IOException {
		...
		FileRegion fileRegion = (FileRegion)queue.poll();
		...
		_assertZipStream(
			path, new UnsyncByteArrayInputStream(_readFileRegion(fileRegion)));
	}
