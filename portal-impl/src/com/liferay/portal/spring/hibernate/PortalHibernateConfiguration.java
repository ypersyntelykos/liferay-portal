...
public class PortalHibernateConfiguration extends LocalSessionFactoryBean {
	...
	public void setImportedClassNamePattern(String pattern) {
		_importedClassNamePattern = Pattern.compile(pattern);
	}
	...
	protected SessionFactory newSessionFactory(Configuration configuration)
		...
		SessionFactoryImplementor sessionFactoryImplementor =
			(SessionFactoryImplementor)super.newSessionFactory(configuration);

		try {
			Field field = ReflectionUtil.getDeclaredField(
				sessionFactoryImplementor.getClass(), "queryPlanCache");

			SessionFactoryImplementor wrappedSessionFactoryImplementor =
				_wrapSessionFactoryImplementor(
					sessionFactoryImplementor, configuration.getImports());

			field.set(
				sessionFactoryImplementor,
				new QueryPlanCache(wrappedSessionFactoryImplementor));

			return wrappedSessionFactoryImplementor;
		}
		catch (Exception e) {
			if (_log.isWarnEnabled()) {
	...
	private Pattern _importedClassNamePattern;
	private boolean _mvccEnabled = true;

	private static class NoPatternSessionFactoryDelegate {

		public NoPatternSessionFactoryDelegate(Map<String, String> imports) {
	...
	private static class PatternedSessionFactoryDelegate
		...
		private PatternedSessionFactoryDelegate(
			Map<String, String> imports, Pattern importedClassNamePattern,
			SessionFactoryImplementor sessionFactoryImplementor) {

			super(imports);

			_importedClassNamePattern = importedClassNamePattern;
