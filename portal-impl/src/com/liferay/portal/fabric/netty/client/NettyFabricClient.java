...
public class NettyFabricClient implements FabricClient {
	...
	@Override
	public NettyFabricConnectionWatchDog connect(SocketAddress socketAddress) {
		...
		NettyFabricConnectionWatchDog previousNettyFabricConnectionWatchDog =
			nettyFabricConnectionWatchDogs.putIfAbsent(
				socketAddress, nettyFabricConnectionWatchDog);

		if (previousNettyFabricConnectionWatchDog != null) {
			nettyFabricConnectionWatchDog.stopWatching();
	...
	public Map<SocketAddress, FabricConnectionWatchDog>
		...
		return Collections.
			<SocketAddress, FabricConnectionWatchDog>unmodifiableMap(
	...
	public NoticeableFuture<?> shutdown() {
		...
		try {
			for (Map.Entry<SocketAddress, NettyFabricConnectionWatchDog>
				...
				if (nettyFabricConnection != null) {
					try {
						NettyUtil.syncFully(nettyFabricConnection.disconnect());
					}
					catch (Exception e) {
						_log.error(
							"Unable to stop Netty fabric connection watch " +
								"dog on address " + socketAddress, e);
					}
				}
			}
			...
			future.addListener(
				new SyncShutdownFutureListener(defaultNoticeableFuture));
		}
	...
	protected final ConcurrentMap<SocketAddress, NettyFabricConnectionWatchDog>
		nettyFabricConnectionWatchDogs = new ConcurrentHashMap<>();
	...
	protected class SyncShutdownFutureListener
		...
		protected SyncShutdownFutureListener(
			DefaultNoticeableFuture<?> defaultNoticeableFuture) {

			this.defaultNoticeableFuture = defaultNoticeableFuture;
		}

		protected final DefaultNoticeableFuture<?> defaultNoticeableFuture;
