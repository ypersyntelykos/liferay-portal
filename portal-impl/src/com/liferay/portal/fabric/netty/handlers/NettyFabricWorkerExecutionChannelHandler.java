...
public class NettyFabricWorkerExecutionChannelHandler
	...
	protected void channelRead0(
		...
		FabricResourceMappingVisitor fabricResourceMappingVisitor =
			new FabricResourceMappingVisitor(
				InputResource.class, repositoryPath.toFile());
		...
		for (Map.Entry<File, File> entry : inputResourceMap.entrySet()) {
			...
			FileHelperUtil.move(noticeableFuture.get(), mappedFile.toPath());
		}
	...
	protected String convertClassPath(String classPath) {
		...
		for (String pathString :
			...
			Path filePath;
			try {
				filePath = _repository.getFile(pathString, false).get();
			}
			catch (Exception e) {
