...
public class ObjectGraphUtil {
	...
	public static abstract class AnnotatedFieldMappingVisitor
		...
		public AnnotatedFieldMappingVisitor(
			Set<Class<?>> linkedTypeGraphClasses,
			Set<Class<? extends Annotation>> annotationClasses,
			Set<Class<?>> fieldTypeClasses) {

			_linkedTypeGraphClasses = linkedTypeGraphClasses;
		...
		public Object visit(Field field, Object target) throws Exception {
			...
			if ((value == null) ||
				!isLinkedTypeGraphClass(field.getDeclaringClass())) {
		...
		protected boolean isLinkedTypeGraphClass(Class<?> clazz) {
			for (Class<?> linkedTypeGraphClass : _linkedTypeGraphClasses) {
				if (linkedTypeGraphClass.isAssignableFrom(clazz)) {
					return true;
				}
			}
		...
		private final Set<Class<?>> _linkedTypeGraphClasses;
